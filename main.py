...
def parse_args() -> argparse.Namespace:
	...
	parser.add_argument(
		"--mode",
		choices=["stream", "range"],
		default="stream",
		help="stream: append runtime labeled rows each window; range: one-shot export",
	)
	...
	parser.add_argument(
		"--window-time",
		default="30s",
		help="Runtime loop interval for stream mode, default 30s",
	)
	parser.add_argument(
		"--max-windows",
		type=int,
		default=0,
		help="Stop stream mode after N windows (0 means run forever)",
	)
	parser.add_argument(
		"--state-file",
		default="LAB_CENTER/web/agent/chaos_runtime_state.json",
		help="Runtime chaos state JSON written by chaos runner",
	)
	parser.add_argument(
		"--state-max-age",
		default="20m",
		help="Maximum age for active runtime state before ignoring it (default 20m)",
	)
	parser.add_argument(
		"--rules",
...
def parse_duration_to_seconds(value: str) -> int:
	...
	if not match:
		raise ValueError(f"Invalid duration format: {value}. Expected 30s, 5m, 1h, 1d")
...
def has_missing_critical_metrics(row: pd.Series, metric_names: Iterable[str]) -> bool:
	# Mark insufficient_data only when all critical metrics are missing and
	# there are no proxy signals available for fallback classification.
	critical_missing = all(pd.isna(row.get(metric)) for metric in metric_names)
	...
	proxy_available = any(not pd.isna(row.get(metric)) for metric in proxy_metrics)
	return not proxy_available


def read_runtime_label(state_path: Path, max_age_seconds: int) -> Optional[str]:
	...
	started_at = parse_iso_utc(payload.get("started_at"))
	duration_seconds = None
	duration_text = payload.get("duration")
	if isinstance(duration_text, str) and duration_text.strip():
		try:
			duration_seconds = parse_duration_to_seconds(duration_text.strip())
		except ValueError:
			duration_seconds = None

	if started_at is not None:
		now_utc = datetime.now(tz=timezone.utc)
		grace_seconds = max(30, min(300, max_age_seconds // 2))
		effective_ttl = max_age_seconds
		if duration_seconds is not None:
			effective_ttl = max(effective_ttl, duration_seconds + grace_seconds)
		if (now_utc - started_at).total_seconds() > effective_ttl:
			print(
				f"[WARN] Ignore stale runtime state from {started_at.isoformat()} "
				f"(age>{effective_ttl}s): {state_path}"
			)
			return None
...
def apply_runtime_override(df: pd.DataFrame, runtime_label: Optional[str]) -> pd.DataFrame:
	if not runtime_label:
		return df
	out = df.copy()
	out["label_root_cause"] = runtime_label
	out["label_confidence"] = 0.99
	out["is_mixed"] = 0
	out["matched_rules"] = out["matched_rules"].apply(lambda x: "R_RUNTIME_EVENT" if not x else f"{x},R_RUNTIME_EVENT")
	out["label_severity"] = out.apply(lambda r: infer_severity(r, runtime_label), axis=1)
	return out


def load_seen_keys(output_path: Path) -> Set[Tuple[str, str]]:
	if not output_path.exists() or output_path.is_dir():
		return set()
	try:
		existing = pd.read_csv(output_path, usecols=["timestamp", "instance"])
	except Exception:
		return set()
	seen: Set[Tuple[str, str]] = set()
	for _, row in existing.iterrows():
		seen.add((str(row["timestamp"]), str(row["instance"])))
	return seen


def filter_new_rows(df: pd.DataFrame, seen_keys: Set[Tuple[str, str]]) -> pd.DataFrame:
	if df.empty:
		return df
	mask = []
	for _, row in df.iterrows():
		key = (str(row["timestamp"]), str(row["instance"]))
...
def finalize_columns(labeled_df: pd.DataFrame) -> pd.DataFrame:
	front_cols = [
		"timestamp",
		"instance",
		"window_size",
	...
	return labeled_df[front_cols + other_cols]
...
def run_label_pipeline(
	...
	return apply_rules(feat_df, rules_cfg)


def run_range_mode(args: argparse.Namespace, rules_cfg: Dict[str, Any], alias_to_query: Dict[str, str], required_aliases: List[str]) -> None:
	...
	final_df = finalize_columns(labeled_df)
...
def run_stream_mode(args: argparse.Namespace, rules_cfg: Dict[str, Any], alias_to_query: Dict[str, str], required_aliases: List[str]) -> None:
	window_seconds = parse_duration_to_seconds(args.window_time)
	state_max_age_seconds = parse_duration_to_seconds(args.state_max_age)
	state_path = resolve_existing_path(args.state_file)
	...
	windows = build_windows_from_rules(rules_cfg)
	lookback_seconds = max(windows.values()) * window_seconds + 300
	seen_keys = load_seen_keys(output_path)
	count = 0

	print(f"[INFO] Stream mode started. window={args.window_time}, output={output_path}")
	while True:
		now = datetime.now(tz=timezone.utc)
		end = now
		...
		try:
			...
			print(f"[WARN] Stream window failed: {exc}")
			time.sleep(window_seconds)
			continue

		if labeled_df.empty:
			print("[WARN] No data in current window")
			time.sleep(window_seconds)
			continue

		latest_ts = labeled_df["timestamp"].max()
		batch_df = labeled_df[labeled_df["timestamp"] == latest_ts].copy()
		runtime_label = read_runtime_label(state_path, max_age_seconds=state_max_age_seconds)
		batch_df = apply_runtime_override(batch_df, runtime_label)
		batch_df = finalize_columns(batch_df)
		batch_df = filter_new_rows(batch_df, seen_keys)

		if not batch_df.empty:
			write_header = not output_path.exists() or output_path.stat().st_size == 0
			batch_df.to_csv(output_path, mode="a", header=write_header, index=False)
			print(f"[INFO] Appended {len(batch_df)} rows for {latest_ts}")

		count += 1
		if args.max_windows > 0 and count >= args.max_windows:
			print("[INFO] Reached max windows, stopping stream mode")
			break
...
def main() -> None:
	...
	if missing:
		raise RuntimeError("Missing PromQL mapping for metric aliases: " + ", ".join(sorted(missing)))

	if args.mode == "range":
		run_range_mode(args, rules_cfg, alias_to_query, required_aliases)
	else:
		run_stream_mode(args, rules_cfg, alias_to_query, required_aliases)

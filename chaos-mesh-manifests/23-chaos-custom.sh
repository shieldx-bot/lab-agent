...
STATE_FILE="${CHAOS_RUNTIME_STATE_FILE:-${ROOT_DIR}/chaos_runtime_state.json}"
...
fault_to_label() {
  case "${FAULT}" in
    disk-latency) echo "disk_latency" ;;
    disk-read-fault|disk-write-fault) echo "disk_error" ;;
    disk-saturation) echo "disk_saturation" ;;
    net-delay) echo "net_delay" ;;
    net-loss) echo "net_loss" ;;
    net-duplicate) echo "net_duplicate" ;;
    net-corrupt) echo "net_corrupt" ;;
    net-bandwidth) echo "net_bandwidth_limit" ;;
    net-partition) echo "net_partition" ;;
    dns-error|dns-timeout) echo "dns_fault" ;;
  ...
  cat >"${STATE_FILE}" <<EOF
{
  ...
  "source": "23-chaos-custom.sh"
}
EOF
  echo "[INFO] runtime state updated: ${STATE_FILE}"
}
...
EOF
  echo "[INFO] runtime state cleared: ${STATE_FILE}"
}
